    ...
    <div class="page-header mb-4">
        <div>
            ...
            <p class="page-subtitle">
                Kelola daftar kategori produk fashion, kelola kode unik, dan status publikasi website.
            </p>
        </div>
        ...
        <form action="{{ route('admin.kategori.index') }}" method="GET" class="search-form">
            <div class="search-input-group">
                ...
                <input 
                    ...
                    placeholder="Cari berdasarkan nama, kode kategori, atau slug..."
                    value="{{ request('search') }}"
                >
                @if(request('search'))
                    <a href="{{ route('admin.kategori.index') }}" class="btn-clear-search" title="Hapus Pencarian">
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="code-badge">
                                <i class="fa-solid fa-hashtag me-1"></i>{{ $item->kode_kategori }}
                            </span>
                        </td>
                        ...
                        <td>
                            <div class="action-buttons justify-content-end">
                                <a href="{{ route('admin.kategori.edit', $item->id) }}" class="btn-edit" title="Ubah Kategori">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>

                                <form action="{{ route('admin.kategori.destroy', $item->id) }}" method="POST" class="delete-form d-inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state">
                                ...
                                <h3>Data Kategori Kosong</h3>
                                <p>
                                    @if(request('search'))
                                        Tidak ditemukan kategori dengan kata kunci "{{ request('search') }}". Coba cari kata kunci lain.
                                    @else
                                        Belum ada data kategori pakaian yang tersimpan di sistem.
                                    @endif
                                </p>
                                @if(request('search'))
                                    <a href="{{ route('admin.kategori.index') }}" class="btn-add m-auto mt-3" style="width: max-content;">
