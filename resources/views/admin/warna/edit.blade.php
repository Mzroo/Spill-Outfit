...
@section('title', 'Ubah Detail Warna')
...
<div class="container-fluid">
    ...
    <div class="page-header mb-4">
        <div>
            <h1 class="page-title">Ubah Detail Warna</h1>
            <p class="page-subtitle">Perbarui nama varian kain, sesuaikan saturasi kode warna HEX, atau kelola status tampilnya.</p>
        </div>
                ...
                <div class="col-lg-6">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text"
                               ...
                               placeholder="Contoh: Onyx Black, Creamy White"
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex gap-2 align-items-center">
                            <input type="color"
                                   ...
                                   value="{{ old('kode_warna', $warna->kode_warna ?? '#8C6A2F') }}"
                                   title="Klik untuk mengubah warna">
                            <div class="text-muted small ps-2">
                                Klik kotak warna di atas untuk menggeser palet atau mengubah saturasi warna secara visual.
                            </div>
                        </div>
                        @error('kode_warna')
                            <div class="invalid-feedback fw-semibold mt-2">{{ $message }}</div>
                    ...
                    <div class="mb-5">
                        ...
                        <select name="status" class="form-select custom-input">
                            <option value="aktif" {{ old('status', $warna->status) == 'aktif' ? 'selected' : '' }}>Aktif (Tersedia untuk variasi produk)</option>
                            <option value="nonaktif" {{ old('status', $warna->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif (Disembunyikan sementara)</option>
                        </select>
                    </div>
                    ...
                    <div class="sticky-preview-wrapper">
                        <div class="preview-card-frame">
                            ...
                            <div class="preview-content-center">
                                ...
                                <h4 class="preview-color-name" id="previewNama">{{ $warna->nama }}</h4>

                                <div class="preview-divider"></div>

                                <span class="preview-hex-badge" id="previewKode">{{ $warna->kode_warna ?? '#8C6A2F' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Set state awal berdasarkan data database / old value
        previewColor.style.backgroundColor = kodeInput.value;
        previewKode.innerText = kodeInput.value.toUpperCase();
        previewNama.innerText = namaInput.value.trim() || 'Nama Warna';

        // Event listener saat mengubah nama
        namaInput.addEventListener('input', function () {
            previewNama.innerText = this.value.trim() || 'Nama Warna';
        });

        // Event listener saat mengubah palet warna
