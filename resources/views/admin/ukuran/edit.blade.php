                ...
                <div class="col-lg-7">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text"
                               ...
                               required>
                        <small class="text-muted d-block mt-1">Sistem akan otomatis menyimpan kode ini dalam format huruf kapital.</small>
                        @error('kode')
                            <div class="invalid-feedback fw-semibold mt-2">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <select name="status" class="form-select custom-input">
                            <option value="aktif" {{ old('status', $ukuran->status) == 'aktif' ? 'selected' : '' }}>Aktif (Ditampilkan di form produk)</option>
                            <option value="nonaktif" {{ old('status', $ukuran->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif (Disembunyikan sementara)</option>
                        </select>
                    </div>
                        ...
                        <div class="preview-card-frame">
                            ...
                            <div class="preview-content-center">
                                <div class="preview-size-badge-box mb-3">
                                    <span id="previewKode">{{ $ukuran->kode }}</span>
                                </div>

                                <h4 class="preview-size-name" id="previewNama">{{ $ukuran->nama }}</h4>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        namaInput.addEventListener('input', function () {
            previewNama.innerText = this.value.trim() || 'Nama Ukuran';
        });

        kodeInput.addEventListener('input', function () {
            previewKode.innerText = this.value.trim().toUpperCase() || '-';
        });

        keteranganInput.addEventListener('input', function () {
            previewKeterangan.innerText = this.value.trim() || 'Belum ada keterangan';
        });
    });
</script>
