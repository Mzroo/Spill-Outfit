...
<div class="container-fluid">
    ...
    <div class="page-header mb-4">
        <div>
            ...
            <p class="page-subtitle">
                Kelola daftar produsen apparel, kontrol hak autentikasi merek, dan status kemitraan website.
            </p>
        </div>
    ...
    <div class="utility-bar mb-4">
        <form action="{{ route('admin.brand.index') }}" method="GET" class="search-form">
            <div class="search-input-group">
                ...
                <input 
                    ...
                    placeholder="Cari berdasarkan nama merek, kode brand, atau slug..."
                    value="{{ request('search') }}"
                >
                @if(request('search'))
                    <a href="{{ route('admin.brand.index') }}" class="btn-clear-search" title="Hapus Pencarian">
    ...
    <div class="custom-card">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-center text-muted font-monospace">
                            {{ ($brand->currentPage() - 1) * $brand->perPage() + $loop->iteration }}
                        </td>

                        <td>
                            <span class="code-badge">
                                <i class="fa-solid fa-hashtag me-1"></i>{{ $item->kode_brand }}
                            </span>
                        </td>

                        <td>
                            @if($item->logo)
                                <img src="{{ asset('storage/' . $item->logo) }}" class="brand-logo" alt="{{ $item->nama }}">
                            @else
                        ...
                        <td>
                            ...
                            @if($item->deskripsi)
                                <small class="text-muted d-block text-truncate table-desc-text" style="max-width: 280px;">
                                    {{ $item->deskripsi }}
                                </small>
                            @endif
                        </td>
                        ...
                        <td>
                            <span class="slug-badge">
                                <i class="fa-solid fa-link me-1" style="font-size: 11px;"></i>{{ $item->slug }}
                            </span>
                        </td>
                        ...
                        <td>
                            <div class="action-buttons justify-content-end">
                                <a href="{{ route('admin.brand.edit', $item->id) }}" class="btn-edit" title="Ubah Brand">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>

                                <form action="{{ route('admin.brand.destroy', $item->id) }}" method="POST" class="delete-form d-inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus merek brand ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-delete" title="Hapus Brand">
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state">
                                ...
                                <h3>Data Brand Kosong</h3>
                                <p>
                                    @if(request('search'))
                                        Tidak ditemukan brand / merek dengan kata kunci "{{ request('search') }}". Coba cari nama lainnya.
                                    @else
                                        Belum ada data partner brand apparel resmi yang tersimpan di sistem.
                                    @endif
                                </p>
                                @if(request('search'))
                                    <a href="{{ route('admin.brand.index') }}" class="btn-add m-auto mt-3" style="width: max-content;">
        ...
        @if($brand->hasPages())
            <div class="custom-pagination-wrapper mt-4">
                {{ $brand->appends(request()->query())->links('pagination::bootstrap-5') }}
            </div>
        @endif
    </div>
</div>
...
/* ================= PERBAIKAN CODE BADGE DI INDEX (ANTI KEPOTONG) ================= */
.code-badge {
    background: #faf6ed;
    color: #8C6A2F;
    padding: 6px 14px;
